...
wxThread::ExitCode SocketServerThread::Entry()
{
	...
	if (should_shutdown_lock.IsOk() == true) should_shutdown = false;
	else MyPrintWithDetails("Error: Can't get shutdown lock");
	}
	...
	for (short int current_port = START_PORT; current_port <= END_PORT; current_port++)
	{
		...
		if (current_port == END_PORT)
		{
			...
			if (server_is_running_lock.IsOk() == true) parent_pointer->server_is_running = false;
			else MyPrintWithDetails("Error: Can't get server is running lock");
			}

			{
			wxMutexLocker socket_server_lock(parent_pointer->server_mutex);
			if (socket_server_lock.IsOk() == true) socket_server = NULL;
			else MyPrintWithDetails("Error: Can't get server lock");
			}
			//Destroy();
		...
		if (socket_server_lock.IsOk() == true)
		{
			...
			if ( socket_server->IsOk())
			{
				  ...
				  { // for mutex
				  wxMutexLocker server_is_running_lock(parent_pointer->server_is_running_mutex);
				  if (server_is_running_lock.IsOk() == true) parent_pointer->server_is_running = true;
				  else MyPrintWithDetails("Error: Can't get server is running lock");;
				  }
	...
	while (1==1)
	{
		...
		else MyPrintWithDetails("Error: Server is NULL");
		}
...
SocketCommunicator::SocketCommunicator()
{
	...
	if (server_is_running_lock.IsOk() == true) server_is_running = false;
	else MyPrintWithDetails("Error: Can't get server is running lock");
	}

	{ // mutex
	wxMutexLocker monitor_is_running_lock(monitor_is_running_mutex);
	if (monitor_is_running_lock.IsOk() == true) monitor_is_running = false;
	else MyPrintWithDetails("Error: Can't get monitor is running lock");
	}
}
...
//	MyDebugAssertFalse(server_is_running, "Error: trying to start a server, but a server is already running");
	...
	while (time_waited < 50)
	{
		...
		if (server_is_running_lock.IsOk() == true)
		{
			if (server_is_running == true) break;
		}
		else MyPrintWithDetails("Error: Can't get server is running lock");;
		}
...
//	MyDebugAssertTrue(server_thread != NULL, "Error: Shutdown Server called, when server thread = NULL");
	...
	if (server_thread != NULL) server_thread->Delete();
	server_thread = NULL;
	...
	while (time_waited < 50)
	{
		...
		if (server_is_running_lock.IsOk() == true)
		{
			if (server_is_running == false) break;
		}
		else MyPrintWithDetails("Error: Can't get server is running lock");;
		}
	...
	if (time_waited == 50) wxPrintf("Timed out waiting for server shutdown\n");

}

void SocketCommunicator::ShutDownSocketMonitor()
{
	if (socket_monitor_thread != NULL) socket_monitor_thread->Delete();
	...
	while (time_waited < 50)
	{
		...
		{
			if (monitor_is_running == false) break;
		}
		else MyPrintWithDetails("Error: Can't get monitor is running lock");;
		}
		wxMilliSleep(100);
	...
	if (time_waited == 50) wxPrintf("Timed out waiting for socket monitor shutdown\n");
...
short int SocketCommunicator::ReturnServerPort()
{
	...
	if (server_is_running_lock.IsOk() == true)
	{
		if (server_is_running == true && server_thread != NULL) return server_thread->my_port;
	}
	else MyPrintWithDetails("Error: Can't get server is running lock");
...
wxString SocketCommunicator::ReturnServerPortString()
{
	...
	if (server_is_running_lock.IsOk() == true)
	{
		if (server_is_running == true && server_thread != NULL) return server_thread->my_port_string;
	}
	else MyPrintWithDetails("Error: Can't get server is running lock");
...
wxArrayString SocketCommunicator::ReturnServerAllIpAddresses()
{
	...
	if (server_is_running_lock.IsOk() == true)
	{
		if (server_is_running == true && server_thread != NULL) return server_thread->all_my_ip_addresses;
	}
	else MyPrintWithDetails("Error: Can't get server is running lock");
...
void SocketCommunicator::MonitorSocket(wxSocketBase *socket_to_monitor)
{
	...
	if (monitor_is_running == false)
	{
		...
		while (time_waited < 50)
		{
			...
			if (monitor_is_running_lock.IsOk() == true)
			{
				if (monitor_is_running == true) break;
			}
			else MyPrintWithDetails("Error: Can't get monitor is running lock");;
			}
	...
	else monitor_is_running_mutex.Unlock();
	...
	else MyPrintWithDetails("Error: Can't get add sockets lock");
	}
...
void SocketCommunicator::StopMonitoringAndDestroySocket(wxSocketBase *socket_to_monitor)
{
	...
	else MyPrintWithDetails("Error: Can't get remove and destroy socket lock");
}
...
wxThread::ExitCode SocketClientMonitorThread::Entry()
{
	...
	if (monitor_is_running_lock.IsOk() == true) parent_pointer->monitor_is_running = true;
	else MyPrintWithDetails("Error: Can't get server is running lock");;
	}
	...
	while (1==1)
	{
				...
				if (monitored_sockets[socket_counter]->IsOk() == true && monitored_sockets[socket_counter]->IsConnected() == true)
				{
					...
					if (monitored_sockets[socket_counter]->WaitForRead(0, 0) == true)
					{
						...
						if (ReadFromSocket(monitored_sockets[socket_counter], &socket_input_buffer, SOCKET_CODE_SIZE, true, "SendSocketJobType", FUNCTION_DETAILS_AS_WXSTRING) == true)
						{
							...
							if (memcmp(socket_input_buffer, socket_you_are_the_leader, SOCKET_CODE_SIZE) == 0)
							{
								JobPackage *temp_package = new JobPackage;
								if (temp_package->ReceiveJobPackage(monitored_sockets[socket_counter]) == true)
								{
							...
							if (memcmp(socket_input_buffer, socket_you_are_a_follower, SOCKET_CODE_SIZE) == 0)
							{
								...
								leader_ip_address = ReceivewxStringFromSocket(monitored_sockets[socket_counter], no_error);

								if (no_error == true)
								{
									leader_port_string = ReceivewxStringFromSocket(monitored_sockets[socket_counter], no_error);

								}

								if (no_error == true)
								{
									parent_pointer->brother_event_handler->CallAfter(std::bind(&SocketCommunicator::HandleSocketYouAreAFollower, parent_pointer, monitored_sockets[socket_counter], leader_ip_address, leader_port_string));
								}
								else
								{
									// socket is not ok.. pass on a message to the handler and remove it..
							...
							if (memcmp(socket_input_buffer, socket_i_have_info, SOCKET_CODE_SIZE) == 0)
							{
								...
								// pass the info on..
								if (no_error == true) parent_pointer->brother_event_handler->CallAfter(std::bind(&SocketCommunicator::HandleSocketIHaveInfo, parent_pointer, monitored_sockets[socket_counter], info_message));
								else
								{
									// socket is not ok.. pass on a message to the handler and remove it..
									parent_pointer->brother_event_handler->CallAfter(std::bind(&SocketCommunicator::HandleSocketDisconnect,parent_pointer, monitored_sockets[socket_counter]));
							...
							if (memcmp(socket_input_buffer, socket_job_result, SOCKET_CODE_SIZE) == 0)
							{
								...
								JobResult *temp_job = new JobResult;
								if (temp_job->ReceiveFromSocket(monitored_sockets[socket_counter]) == true) parent_pointer->brother_event_handler->CallAfter(std::bind(&SocketCommunicator::HandleSocketJobResult,parent_pointer, monitored_sockets[socket_counter], temp_job));
								else
								{
									// socket is not ok.. pass on a message to the handler and remove it..
									parent_pointer->brother_event_handler->CallAfter(std::bind(&SocketCommunicator::HandleSocketDisconnect,parent_pointer, monitored_sockets[socket_counter]));
								...
								if (ReadFromSocket(monitored_sockets[socket_counter], details, sizeof(int) * 3, true, "SendResultImageDetailsFromFollowerToLeader", FUNCTION_DETAILS_AS_WXSTRING) == true)
								{
									...
									if (ReadFromSocket(monitored_sockets[socket_counter], image_to_write->real_values, image_to_write->real_memory_allocated * sizeof(float), true, "SendResultImageDataFromFollowerToLeader", FUNCTION_DETAILS_AS_WXSTRING) == true)
									{
										...
										if ( no_error == true)
										{
											...
											// provided much faster than it can be written then the leader's memory filled up.  In order to fix that, I am changing it so that image is directly written by the socket communicator.
											// this is also no ideal, as the leader will freeze to all events while writing the image, but we shall see how it goes.
							...
							if (memcmp(socket_input_buffer, socket_program_defined_result, SOCKET_CODE_SIZE) == 0)
							{
								float *data_array;
								int details[3];
		...
		if (number_with_data == 0) wxMilliSleep(250);
	}
